export default {
    back: require('../assets/images/back.png'),
    book: require('../assets/images/book.png'),
    bookStack: require('../assets/images/bookStack.png'),
    gita: require('../assets/images/gita.png'),
    menu: require('../assets/images/menus.png'),
    more: require('../assets/images/more.png'),
    reader: require('../assets/images/reader.png'),
    search: require('../assets/images/search.png'),
    downloading: require('../assets/images/bookDownloading.png'),
    bookPlaceholder: require('../assets/images/bookPlaceholder.png'),
    close: require('../assets/images/close.png'),
    loading: require('../assets/images/loadingBooks.gif'),
};